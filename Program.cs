    ...
    class Program
    {
        ...
        private static async Task HandleCallbackQueryAsync(ITelegramBotClient botClient, CallbackQuery callback)
        {
            ...
            try
            {
                ...
                if (data.StartsWith("schedule_"))
                {
                    ...
                    {
                        await botClient.SendTextMessageAsync(userId, "⚠️ Ошибка: сессия прервана. Начните заново с /create_note");
                        await botClient.AnswerCallbackQueryAsync(callback.Id);
                        return;
                    }
                    ...
                    if (scheduleType == "OneTime")
                    {
                        ...
                        await botClient.SendTextMessageAsync(userId, "🕐 Введите время в формате ЧЧ:ММ (например, 14:30):");
                    }
                    else
                    {
                        switch (scheduleType)
                        {
                            ...
                            case "Interval":
                                ...
                                await botClient.SendTextMessageAsync(userId, "⏰ Введите интервал в минутах или Ч:ММ:");
                                break;
                            case "Weekly":
                                _userStates[userId] = (UserState.EnteringWeekDays, tempReminder);
                                await botClient.SendTextMessageAsync(userId, "📆 Введите дни недели через запятую (1=Пн, ..., 7=Вс):");
                                break;
                        }
                    }
                }
                ...
                else if (data.StartsWith("edit_title_"))
                {
                    ...
                    await botClient.SendTextMessageAsync(userId, $"✏️ Текущее название: \"{reminder.Title}\"\nВведите новое название:");
                }
                ...
                else if (data.StartsWith("edit_text_"))
                {
                    ...
                    await botClient.SendTextMessageAsync(userId, $"✏️ Текущий текст: \"{reminder.Text}\"\nВведите новый текст:");
                }
                ...
                else if (data.StartsWith("edit_schedule_"))
                {
                    ...
                    await botClient.SendTextMessageAsync(userId, "Выберите новый тип расписания:", replyMarkup: scheduleKeyboard);
                }
                ...
                else if (data.StartsWith("editsched_"))
                {
                    ...
                    switch (scheduleType)
                    {
                        case "Daily":
                        case "OneTime":
                            await botClient.SendTextMessageAsync(userId, "🕐 Введите новое время в формате ЧЧ:ММ:");
                            break;
                        case "Interval":
                            await botClient.SendTextMessageAsync(userId, "⏰ Введите новый интервал в минутах или Ч:ММ:");
                            break;
                        case "Weekly":
                            await botClient.SendTextMessageAsync(userId, "📆 Введите новые дни недели через запятую (1=Пн, ..., 7=Вс):");
                            break;
                    }
                }
                ...
                else if (data.StartsWith("edit_prenotif_"))
                {
                    ...
                    var current = string.IsNullOrEmpty(reminder.PreNotificationOffsets) ? "не заданы" : reminder.PreNotificationOffsets;
                ...
                else if (data.StartsWith("toggle_status_"))
                {
                    ...
                    if (success)
                    {
                        ...
                        {
                            _userStates[userId] = (UserState.EditingReminderMenu, reminder);
                            await botClient.SendTextMessageAsync(userId, $"✅ Статус изменён: {(reminder.IsActive ? "🟢 Активно" : "⚫ Неактивно")}");
                            await SendEditMenuAsync(botClient, userId, reminder);
                        }
                    }
                    else
                ...
                else if (data.StartsWith("delete_"))
                {
                    ...
                    await botClient.SendTextMessageAsync(userId, $"Вы уверены, что хотите удалить \"{reminder.Title}\"?", replyMarkup: confirmKeyboard);
                }
            ...
            catch (Exception ex)
            {
                ...
                await botClient.SendTextMessageAsync(userId, $"️ Ошибка: {ex.Message}\nПопробуйте начать заново.");
            }
        ...
        private static async Task SendEditMenuAsync(ITelegramBotClient botClient, long userId, Reminder reminder)
        {
            ...
            var keyboard = new InlineKeyboardMarkup(new[]
            {
                ...
                new[] { InlineKeyboardButton.WithCallbackData("◀️ Назад к списку (off)", "edit_back") }
            });
            await botClient.SendTextMessageAsync(userId,
        ...
        private static async Task HandleCommandAsync(ITelegramBotClient botClient, Message message, AppUser user, string command)
        {
            ...
            switch (command)
            {
                case "/create_note":
                    ...
                    await botClient.SendTextMessageAsync(userId, "📝 Введите название напоминания (только текст и цифры):");
                    break;
                ...
                case "/edit_note":
                    ...
                    var editButtons = reminders.Select(r =>
                    {
                        ...
                        return InlineKeyboardButton.WithCallbackData($"{status} {r.DisplayOrder}. {r.Title}", $"edit_{r.Id}");
                    }).ToArray();
                    await botClient.SendTextMessageAsync(userId, "Выберите напоминание для редактирования:",
                        replyMarkup: new InlineKeyboardMarkup(editButtons));
                    break;
                ...
                case "/delete_note":
                    ...
                    var deleteButtons = allReminders.Select(r =>
                        InlineKeyboardButton.WithCallbackData($"❌ {r.DisplayOrder}. {r.Title}", $"delete_{r.Id}")
                    ).ToArray();
                    await botClient.SendTextMessageAsync(userId, "Выберите напоминание для удаления:",
                        replyMarkup: new InlineKeyboardMarkup(deleteButtons));
                    break;
        ...
        private static async Task HandleInputAsync(ITelegramBotClient botClient, Message message, AppUser user, string? text)
        {
            ...
            if (state == UserState.EnteringTitle || state == UserState.EditingTitle ||
                state == UserState.EnteringText || state == UserState.EditingText)
            {
                // Для названия и текста разрешаем любые символы
            }
            else if (state != UserState.EnteringTime && state != UserState.EditingSchedule && 
                ...
                switch (state)
                {
                    ...
                    case UserState.EnteringText:
                        ...
                        var scheduleKeyboard = new InlineKeyboardMarkup(new[]
                        {
                            ...
                            new[] { InlineKeyboardButton.WithCallbackData("🔹 Один раз (в разработке)", "schedule_onetime") }
                        });
                    ...
                    case UserState.EnteringInterval:
                        ...
                        var newIntervalReminder = await _reminderService.CreateReminderAsync(
                            ...
                            tempReminder.IntervalHours, tempReminder.IntervalMinutes, tempReminder.WeekDays, 
                            tempReminder.Time, null, user.TimezoneOffsetHours);  // ← Добавлен null
                        _userStates.Remove(userId);
                    ...
                    case UserState.EnteringTime:
                        ...
                        var newReminder = await _reminderService.CreateReminderAsync(
                            userId, tempReminder.Title, tempReminder.Text, tempReminder.ScheduleType, 
                            tempReminder.IntervalHours, tempReminder.IntervalMinutes, tempReminder.WeekDays, 
                            tempReminder.Time, null, user.TimezoneOffsetHours); 
                            _userStates.Remove(userId);
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    ...
                    foreach (var reminder in dueReminders)
                    {
                        ...
                        if (!isPreNotification && reminder.NextTriggerUtc.HasValue && reminder.NextTriggerUtc.Value <= DateTime.UtcNow)
                        {
                            await _botClient.SendTextMessageAsync(
                                reminder.UserId,
            ...
            int? intervalMinutes, string? weekDays, string time, int timezoneOffsetHours = 0)
        {
            ...
            switch (scheduleType)
            {
                ...
                case "Weekly":
                    ...
                    {
                        var days = weekDays.Split(',').Select(d => int.Parse(d.Trim())).ToList();
        ...
        private static string GetDayName(int day)
        {
            return day switch
            {
                1 => "Пн", 2 => "Вт", 3 => "Ср", 4 => "Чт", 5 => "Пт", 6 => "Сб", 7 => "Вс", _ => "?"
            };
        }
            ...
            switch (reminder.ScheduleType)
            {
                ...
                case "Interval":
                    if (reminder.IntervalMinutes.HasValue)
                        return $"Каждые {FormatMinutes(reminder.IntervalMinutes.Value)}";
                    return "Каждые N мин.";
                case "Weekly":
                    var days = reminder.WeekDays?.Split(',').Select(d => GetDayName(int.Parse(d.Trim()))).ToList();
